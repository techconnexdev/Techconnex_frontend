...
export default function CustomerPaymentsPage() {
  ...
  const getPaymentStatusLabel = (status: string) => {
    if (status === "completed") return t("customer.payments.status.completed");
    if (status === "pending") return t("customer.payments.status.pending");
    if (status === "processing")
      return t("customer.payments.status.processing");
    if (status === "failed") return t("customer.payments.status.failed");
    return status;
  };
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600">{t("customer.payments.subtitle")}</p>
          </div>
          <div className="flex gap-3">
            <Button variant="outline">
              <Download className="w-4 h-4 mr-2" />
              {t("customer.payments.export")}
            </Button>
            <Dialog>
              <DialogTrigger asChild>
                <Button>
                  <Plus className="w-4 h-4 mr-2" />
                  {t("customer.payments.addMethod")}
                </Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {t("customer.payments.dialog.addDescription")}
                  </DialogDescription>
                </DialogHeader>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    RM{stats.pendingPayments.toLocaleString()}
                  </p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500">
                    {t("customer.payments.stats.thisMonth")}
                  </p>
                  <p className="text-2xl font-bold">
                    RM{stats.thisMonth.toLocaleString()}
                  </p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-500">
                    {t("customer.payments.stats.totalTransactions")}
                  </p>
                  <p className="text-2xl font-bold">
        ...
        >
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="transactions">
              {t("customer.payments.tabs.transactions")}
            </TabsTrigger>
            <TabsTrigger value="methods">
              {t("customer.payments.tabs.methods")}
            </TabsTrigger>
          </TabsList>
                ...
                <div className="flex flex-col lg:flex-row gap-4">
                  ...
                  <Select value={filterStatus} onValueChange={setFilterStatus}>
                    <SelectTrigger className="w-full lg:w-48">
                      ...
                      <SelectValue
                        placeholder={t(
                          "customer.payments.filter.statusPlaceholder",
                        )}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">
                        {t("customer.payments.filter.statusAll")}
                      </SelectItem>
                      <SelectItem value="completed">
                        {t("customer.payments.status.completed")}
                      </SelectItem>
                      <SelectItem value="pending">
                        {t("customer.payments.status.pending")}
                      </SelectItem>
                      <SelectItem value="processing">
                        {t("customer.payments.status.processing")}
                      </SelectItem>
                      <SelectItem value="failed">
                        {t("customer.payments.status.failed")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <Select value={filterPeriod} onValueChange={setFilterPeriod}>
                    <SelectTrigger className="w-full lg:w-48">
                      ...
                      <SelectValue
                        placeholder={t(
                          "customer.payments.filter.periodPlaceholder",
                        )}
                      />
                    </SelectTrigger>
                    <SelectContent>
                ...
                >
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {t("customer.payments.method.expires", {
                      ...
                      {method.isDefault && (
                        <Badge className="bg-blue-100 text-blue-800">
                          {t("customer.payments.method.default")}
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2">
                      {!method.isDefault && (
                        <Button variant="outline" size="sm">
                          {t("customer.payments.method.setDefault")}
                        </Button>
                      )}
                      <Button variant="outline" size="sm">
                        {t("customer.payments.method.edit")}
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                      ...
                      >
                        {t("customer.payments.method.remove")}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input placeholder={t("customer.payments.form.expiryPlaceholder")} />
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">
            {t("customer.payments.form.cvv")}
          </label>
          <Input placeholder={t("customer.payments.form.cvvPlaceholder")} />
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        <Button>{t("customer.payments.form.submit")}</Button>
      </div>
    </div>
  );
}
