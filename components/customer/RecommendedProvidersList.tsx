...
export function RecommendedProvidersList({
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      {showCacheInfo && cacheInfo?.cachedAt != null && cacheInfo?.nextRefreshAt != null && (
      ...
      {providers.map((provider) => (
        ...
        >
          ...
          <div className={`flex items-start space-x-2 sm:space-x-3 ${compact ? "pr-16" : "pr-20"}`}>
            <Avatar className={compact ? "w-9 h-9 flex-shrink-0" : "w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0"}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs sm:text-sm text-gray-600 truncate">{provider.specialty}</p>
              {Boolean(provider.recommendedFor?.title) && (
                <p className="text-xs text-blue-600 mt-1 font-medium">
                  Recommended for: {provider.recommendedFor?.title}
                </p>
              )}
              {compact && provider.availability != null && provider.availability !== "" && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  />
                  <span className="text-xs text-gray-600 capitalize">{provider.availability}</span>
                </div>
              )}
              {!compact && (
                <>
                  <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                    ...
                    {typeof provider.yearsExperience === "number" && provider.yearsExperience > 0 && (
                      <span className="text-xs text-gray-500">• {provider.yearsExperience} years exp.</span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {provider.workPreference && (
                      <span className="text-xs text-gray-500 capitalize">• {provider.workPreference}</span>
                    )}
                  </div>
                  ...
                  <div className="text-xs sm:text-sm text-blue-800 space-y-1.5 sm:space-y-2">
                    {provider.aiExplanation
                      ...
                      .map((line, index) => {
                        ...
                        const isWarning =
                          cleanLine.includes("⚠️") || cleanLine.includes("Warning");
                        return cleanLine ? (
                          <div
                            key={index}
                            className={`flex items-start gap-2 sm:gap-3 ${
                              isWarning ? "bg-red-50 p-2 rounded border border-red-200" : ""
                            }`}
                          >
                            <span
