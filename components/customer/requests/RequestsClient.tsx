...
export default function RequestsClient({
  ...
  const accept = async (requestId: string) => {
    ...
    try {
      const r1 = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/projects/accept-provider`, {
        method: "POST",
      ...
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/service-requests/${req.serviceRequestId}`, {
        method: "DELETE",
      });
  ...
  const downloadAttachment = async (attachmentUrl: string) => {
    const full = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/${attachmentUrl}`;
    try {
