...
export default function ProfileOverview({
  ...
  const handleAddSocialUrl = () => {
    if (
      newSocialUrl.trim() &&
      !value.customerProfile?.socialLinks?.includes(newSocialUrl.trim())
    ) {
  ...
  const handleAddLanguage = () => {
    if (
      newLanguage.trim() &&
      !value.customerProfile?.languages?.includes(newLanguage.trim())
    ) {
  ...
  const getPlatformName = (url: string) => {
    ...
    if (lowerUrl.includes("facebook"))
      return t("customer.profile.social.facebook");
    if (lowerUrl.includes("instagram"))
      return t("customer.profile.social.instagram");
    if (lowerUrl.includes("linkedin"))
      return t("customer.profile.social.linkedin");
    if (lowerUrl.includes("twitter") || lowerUrl.includes("x.com"))
      return t("customer.profile.social.twitter");
    if (lowerUrl.includes("youtube"))
      return t("customer.profile.social.youtube");
    return t("customer.profile.social.website");
  };
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      toast({
        title: t("customer.profile.image.invalidType"),
        description: t("customer.profile.image.invalidTypeDesc"),
        variant: "destructive",
      });
      e.target.value = "";
      return;
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast({
        title: t("customer.profile.image.tooLarge"),
        description: t("customer.profile.image.tooLargeDesc"),
        variant: "destructive",
      });
      e.target.value = "";
      return;
  ...
  const handleProfileImageCropConfirm = async (croppedFile: File) => {
    ...
    try {
      const result = await uploadCompanyProfileImage(croppedFile);
      onChange({
        ...value,
        customerProfile: {
          ...(value.customerProfile || {}),
      ...
      toast({
        title: t("customer.profile.image.successTitle"),
        description: t("customer.profile.image.successDesc"),
      });
      ...
      toast({
        title: t("customer.profile.image.uploadFailed"),
  ...
  return (
    <div className="space-y-4 sm:space-y-5 lg:space-y-6">
      <Card
        className={
          isEditing ? "ring-2 ring-blue-300 border-blue-200 bg-blue-50/30" : ""
        }
      >
        <CardHeader className="p-4 sm:p-6">
          <CardTitle className="text-base sm:text-lg flex items-center gap-2">
            {t("customer.profile.overview.title")}
            {isEditing && (
              <span className="text-xs font-normal text-blue-700 bg-blue-100 px-2 py-0.5 rounded">
                {t("customer.profile.overview.editable")}
              </span>
            )}
          </CardTitle>
          <CardDescription className="text-xs sm:text-sm">
            {t("customer.profile.overview.description")}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 sm:space-y-5 lg:space-y-6 p-4 sm:p-6 pt-0">
            ...
            <div className="flex-1 space-y-3 sm:space-y-4 w-full">
              <div>
                <Label htmlFor="companyName" className="text-sm sm:text-base">
                  {t("customer.profile.overview.companyName")}
                </Label>
                {isEditing ? (
                  <>
                    <Input
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {t("customer.profile.overview.supportCompanyName")}
                    </p>
                  </>
                ) : (
                  <p className="text-sm sm:text-base text-gray-900 mt-2">
                    {value.name || t("customer.profile.na")}
                  </p>
                )}
              </div>
              <div>
                <Label htmlFor="description" className="text-sm sm:text-base">
                  {t("customer.profile.overview.companyDesc")}
                </Label>
                {isEditing ? (
                  <Textarea
                    id="description"
                    ...
                    placeholder={t("customer.profile.overview.descPlaceholder")}
                    className="text-sm sm:text-base"
                  />
                ) : (
                  <p className="text-sm sm:text-base text-gray-900 mt-2 whitespace-pre-wrap">
                    {value.customerProfile?.description ||
                      t("customer.profile.overview.noDescription")}
                  </p>
                )}
              </div>
              {memberSince && (
                <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                  ...
                  <span>
                    {t("customer.profile.overview.memberSince", {
                      date: memberSince,
                    })}
                  </span>
                </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-base sm:text-lg font-semibold">
              {t("customer.profile.overview.contactInfo")}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <Label htmlFor="email" className="text-sm sm:text-base">
                  {t("customer.profile.overview.email")}
                </Label>
                {isEditing ? (
                  <>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Verify OTP after changing email to save.
                    </p>
                  </>
                ) : (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <p className="text-sm sm:text-base text-gray-900">
                      {value.email || t("customer.profile.na")}
                    </p>
                  </div>
                )}
              </div>
              <div>
                <Label htmlFor="phone" className="text-sm sm:text-base">
                  {t("customer.profile.overview.phone")}
                </Label>
                {isEditing ? (
                  <>
                    <PhoneInputField
                      ...
                      placeholder={t("customer.profile.overview.phonePlaceholder")}
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Verify OTP after changing phone to save.
                    </p>
                  </>
                ) : (
                  <div className="mt-2 space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm sm:text-base text-gray-900">
                        {value.phone || t("customer.profile.na")}
                      </p>
                    </div>
                    {value.phone?.trim() && !isPhoneVerified && (
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          disabled={phoneVerificationBusy}
                        >
          ...
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-base sm:text-lg font-semibold">
              {t("customer.profile.overview.locationWebsite")}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div>
                <Label htmlFor="location" className="text-sm sm:text-base">
                  {t("customer.profile.overview.location")}
                </Label>
                {isEditing ? (
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t(
                        "customer.profile.overview.locationPlaceholder",
                      )}
                    />
                  </div>
                ) : (
              ...
              <div>
                <Label htmlFor="website" className="text-sm sm:text-base">
                  {t("customer.profile.overview.website")}
                </Label>
                {isEditing ? (
                  <>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        placeholder={t(
                          "customer.profile.overview.websitePlaceholder",
                        )}
                      />
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      {t("customer.profile.overview.websiteHint")}
                    </p>
                  </>
                ) : (
                  <div className="mt-2">
            ...
            <div className="space-y-3 sm:space-y-4">
              <div>
                <h3 className="text-base sm:text-lg font-semibold mb-1 sm:mb-2">
                  {t("customer.profile.overview.socialTitle")}
                </h3>
                <p className="text-xs sm:text-sm text-gray-600">
                  {t("customer.profile.overview.socialHint")}
                </p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2">
                <Input
                  ...
                  placeholder={t("customer.profile.overview.socialPlaceholder")}
                  type="url"
                  onKeyPress={(e) =>
                    e.key === "Enter" &&
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {t("customer.profile.overview.add")}
                  </span>
                </Button>
              </div>

              {value.customerProfile?.socialLinks &&
                value.customerProfile.socialLinks.length > 0 && (
                  <div className="space-y-2 sm:space-y-3">
                    <Label className="text-sm sm:text-base font-medium">
                      {t("customer.profile.overview.socialLinksLabel", {
                        n: String(value.customerProfile.socialLinks.length),
                      })}
                    </Label>
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              ...
                              aria-label={t(
                                "customer.profile.overview.removeSocialAria",
                              )}
                            >
              ...
              {(!value.customerProfile?.socialLinks ||
                ...
                <div className="text-center py-6 sm:py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
                  ...
                  <p className="text-sm sm:text-base font-medium mb-1">
                    {t("customer.profile.overview.noSocialTitle")}
                  </p>
                  <p className="text-xs sm:text-sm">
                    {t("customer.profile.overview.noSocialHint")}
                  </p>
                </div>
              )}
            </div>
          ...
          {isEditing && (
            <div className="space-y-3 sm:space-y-4">
              <div>
                <h3 className="text-base sm:text-lg font-semibold mb-1 sm:mb-2">
                  {t("customer.profile.overview.languagesTitle")}
                </h3>
                <p className="text-xs sm:text-sm text-gray-600">
                  {t("customer.profile.overview.languagesHint")}
                </p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2">
                <Input
                  ...
                  placeholder={t("customer.profile.overview.langPlaceholder")}
                  onKeyPress={(e) =>
                    e.key === "Enter" &&
              ...
              {value.customerProfile?.languages &&
                value.customerProfile.languages.length > 0 && (
                  <div className="space-y-2 sm:space-y-3">
                    <Label className="text-sm sm:text-base font-medium">
                      {t("customer.profile.overview.languagesLabel", {
                        n: String(value.customerProfile.languages.length),
                      })}
                    </Label>
                      ...
                      {value.customerProfile.languages.map((lang, index) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            aria-label={t(
                              "customer.profile.overview.removeLangAria",
                            )}
                          >
          ...
          {!isEditing && (
            <div className="space-y-3 sm:space-y-4">
              ...
              value.customerProfile.languages.length > 0 ? (
                ...
                <p className="text-sm sm:text-base text-gray-500 italic">
                  {t("customer.profile.overview.noLanguages")}
                </p>
              )}
            </div>
          )}
