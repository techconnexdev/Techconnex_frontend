      ...
      <CardContent className="p-4 sm:p-6">
        {loading ? (
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 p-3 sm:p-4 border rounded-lg">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="min-w-0">
                <p className="font-medium text-sm sm:text-base">
                  {t("admin.users.resume.uploaded")}
                </p>
                <p className="text-xs sm:text-sm text-gray-500">
                  {t("admin.users.resume.uploadedOn", {
            ...
            >
              <ExternalLink className="w-3.5 h-3.5 sm:h-4 sm:w-4 mr-1.5 sm:mr-2" />
              {t("admin.users.resume.download")}
            </Button>
          </div>
        ) : (
          <div className="text-center py-6 sm:py-8 text-gray-500">
            <FileText className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-3 sm:mb-4 text-gray-300" />
            <p className="text-sm sm:text-base">{t("admin.users.resume.empty")}</p>
          </div>
        )}
      </CardContent>
    </Card>
