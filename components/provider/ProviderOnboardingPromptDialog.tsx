...
export function ProviderOnboardingPromptDialog({ open, onOpenChange }: Props) {
  ...
  const steps = useMemo(
    () => [
      {
        icon: FileText,
        labelKey: "provider.onboarding.steps.profileCv" as const,
        shortKey: "provider.onboarding.prompt.short.profileCv" as const,
      },
      {
        icon: Target,
        labelKey: "provider.onboarding.steps.skillsExperience" as const,
        shortKey: "provider.onboarding.prompt.short.skillsExperience" as const,
      },
      {
        icon: Link2,
        labelKey: "provider.onboarding.steps.portfolioLinks" as const,
        shortKey: "provider.onboarding.prompt.short.portfolioLinks" as const,
      },
      {
        icon: Award,
        labelKey: "provider.onboarding.steps.certifications" as const,
        shortKey: "provider.onboarding.prompt.short.certifications" as const,
      },
      {
        icon: CheckCircle2,
        labelKey: "provider.onboarding.steps.reviewSummary" as const,
        shortKey: "provider.onboarding.prompt.short.reviewSummary" as const,
      },
    ],
    [],
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100%-2rem)] max-w-lg p-0 gap-0 overflow-hidden border-0 shadow-2xl rounded-xl sm:rounded-2xl max-h-[90dvh] sm:max-h-[85vh] flex flex-col">
        ...
        <div className="px-4 py-4 sm:px-6 sm:py-5 space-y-4 sm:space-y-5 bg-white overflow-y-auto flex-1 min-h-0">
          ...
          <div className="flex items-start gap-2.5 sm:gap-3 p-2.5 sm:p-3 rounded-lg sm:rounded-xl bg-emerald-50 border border-emerald-100">
            ...
            <div className="min-w-0">
              <p className="font-semibold text-emerald-900 text-xs sm:text-sm">{t("provider.onboarding.prompt.whyTitle")}</p>
              <ul className="text-xs sm:text-sm text-emerald-800 mt-1 space-y-0.5">
                <li>{t("provider.onboarding.prompt.whyBullet1")}</li>
                <li>{t("provider.onboarding.prompt.whyBullet2")}</li>
                <li>{t("provider.onboarding.prompt.whyBullet3")}</li>
              </ul>
            </div>
          </div>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
              {t("provider.onboarding.prompt.whatYoullFill")}
            </p>
              ...
              {steps.map((step, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <StepIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0" />
                    <span className="font-medium truncate">{t(step.labelKey)}</span>
                    <span className="text-gray-400 hidden sm:inline shrink-0">— {t(step.shortKey)}</span>
                  </li>
                );
              })}
            </ul>
          ...
          <div className="flex items-start gap-2 text-xs sm:text-sm text-gray-500">
            ...
            <span>{t("provider.onboarding.prompt.timeNote")}</span>
          </div>
        </div>
        ...
        <DialogFooter className="px-4 py-3 sm:px-6 sm:py-4 bg-gray-50 border-t flex-col-reverse sm:flex-row gap-2 shrink-0">
          ...
          >
            {t("provider.onboarding.prompt.maybeLater")}
          </Button>
          <Link href="/provider/onboarding" onClick={() => onOpenChange(false)} className="w-full sm:w-auto order-1 sm:order-2">
            <Button type="button" className="w-full bg-blue-600 hover:bg-blue-700">
              {t("provider.onboarding.prompt.ctaCompleteProfile")}
            </Button>
          </Link>
        </DialogFooter>
      </DialogContent>
